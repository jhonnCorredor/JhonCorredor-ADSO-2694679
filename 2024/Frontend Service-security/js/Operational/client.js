    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.status && Array.isArray(response.data)) {
                documents = response.data.map(function(person) {
                    return {
        ...
        function saveClient(){
            ...
            if (isNaN(selectedPersonId) || selectedPersonId === null) {
              console.error("ID de empleado no válido "+ selectedPersonId);
              return;
            }
  ...
  function findById(id) {
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#t.document").val(data.person.typeDocument);
        $("#document").val(data.person.document);
        $('#email').val(data.person.email);
        $('#phone').val(data.person.phone);
        $('#dateOfBirth').val(data.person.dateOfBirth);
        $('#gender').val(data.person.gender);
        $('#address').val(data.person.address);
    
        $("#selected_city_id").val(data.person.municipality.id);
        $("#city_id").val(data.person.municipality.name);
       
        $("#estado").val(data.person.state == true ? 1 : 0);
